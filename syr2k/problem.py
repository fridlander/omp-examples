...
obj = Plopper(dir_path+'/syr2k.c',dir_path)

cs = CS.ConfigurationSpace(seed=1234)
p1 = CSH.CategoricalHyperparameter(name='p1', choices=[' ','#pragma omp #P3','#pragma omp target teams distribute #P3 #P5 #P9 is_device_ptr(A, B, C)','#pragma omp #P4'], default_value=' ')
...
cond0 = CS.InCondition(p3, p1, ['#pragma omp #P3','#pragma omp target teams distribute #P3 #P5 #P9 is_device_ptr(A, B, C)'])
cond1 = CS.EqualsCondition(p5, p1, '#pragma omp target teams distribute #P3 #P5 #P9 is_device_ptr(A, B, C)')
cond2 = CS.EqualsCondition(p9, p1, '#pragma omp target teams distribute #P3 #P5 #P9 is_device_ptr(A, B, C)')
...
if (obj.p2check(dir_path+'/syr2k.c')): #found p2
  print("found p2")
  p2 = CSH.CategoricalHyperparameter(choices=[' ','#pragma omp #P15','#pragma omp target teams distribute #P15 #P17 #P21 map(to:A[0:(ni-1)*(nj-1)]) map(from:C[0:(ni-1)*(nj-1)])', '#pragma omp #P16'], name='p2')
  ...
  cond12 = CS.InCondition(p15, p2, ['#pragma omp #P15', '#pragma omp target teams distribute #P15 #P17 #P21 map(to:A[0:(ni-1)*(nj-1)]) map(from:C[0:(ni-1)*(nj-1)])'])
  cond13 = CS.EqualsCondition(p17, p2, '#pragma omp target teams distribute #P15 #P17 #P21 map(to:A[0:(ni-1)*(nj-1)]) map(from:C[0:(ni-1)*(nj-1)])')
  cond14 = CS.EqualsCondition(p21, p2, '#pragma omp target teams distribute #P15 #P17 #P21 map(to:A[0:(ni-1)*(nj-1)]) map(from:C[0:(ni-1)*(nj-1)])')
