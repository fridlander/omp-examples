...
obj = Plopper(dir_path+'/gemver.c',dir_path)

cs = CS.ConfigurationSpace(seed=1234)
p1 = CSH.CategoricalHyperparameter(name='p1', choices=[' ','#pragma omp #P3','#pragma omp target teams distribute #P3 #P5 #P9 is_device_ptr(A, u1, v1, u2, v2, w, x, y ,z)','#pragma omp #P4'], default_value=' ')
...
cond0 = CS.InCondition(p3, p1, ['#pragma omp #P3','#pragma omp target teams distribute #P3 #P5 #P9 is_device_ptr(A, u1, v1, u2, v2, w, x, y ,z)'])
cond1 = CS.EqualsCondition(p5, p1, '#pragma omp target teams distribute #P3 #P5 #P9 is_device_ptr(A, u1, v1, u2, v2, w, x, y ,z)')
cond2 = CS.EqualsCondition(p9, p1, '#pragma omp target teams distribute #P3 #P5 #P9 is_device_ptr(A, u1, v1, u2, v2, w, x, y ,z)')

cond3 = CS.OrConjunction(CS.EqualsCondition(p4, p1, '#pragma omp #P4'),
                         CS.EqualsCondition(p4, p3, 'parallel for #P4 #P6 #P7'))

cond4 = CS.EqualsCondition(p6, p3, 'parallel for #P4 #P6 #P7')
cond5 = CS.EqualsCondition(p7, p3, 'parallel for #P4 #P6 #P7')
cond6 = CS.EqualsCondition(p8, p3, 'parallel for #P4 #P6 #P7')
...
if (obj.p2check(dir_path+'/gemver.c')): #found p2
  print("found p2")
  p2 = CSH.CategoricalHyperparameter(choices=[' ','#pragma omp #P15','#pragma omp target teams distribute #P15 #P17 #P21 map(to:A[0:(ni-1)*(nj-1)]) map(from:B[0:(ni-1)*(nj-1)])', '#pragma omp #P16'], name='p2')
  ...
  cond12 = CS.InCondition(p15, p2, ['#pragma omp #P15', '#pragma omp target teams distribute #P15 #P17 #P21 map(to:A[0:(ni-1)*(nj-1)]) map(from:B[0:(ni-1)*(nj-1)])'])
  cond13 = CS.EqualsCondition(p17, p2, '#pragma omp target teams distribute #P15 #P17 #P21 map(to:A[0:(ni-1)*(nj-1)]) map(from:B[0:(ni-1)*(nj-1)])')
  cond14 = CS.EqualsCondition(p21, p2, '#pragma omp target teams distribute #P15 #P17 #P21 map(to:A[0:(ni-1)*(nj-1)]) map(from:B[0:(ni-1)*(nj-1)])')
